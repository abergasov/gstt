...
By design, loop iteration over map is not deterministic, so we can't guarantee that messages will be returned in the order in which they were received.
That's why to keep order - it is stored in list. 
...
* with map covered mutex - `network/singlemap/singlemap_tracker.go`
* with sync.Map - `network/syncmap/syncmap_tracker.go`
